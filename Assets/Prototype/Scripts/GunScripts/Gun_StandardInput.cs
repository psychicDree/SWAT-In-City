    ...
    public class Gun_StandardInput : MonoBehaviour
    {
        ...
        void CheckIfWeaponShouldAttack()
        {
            if (Time.time > nextAttack && Time.timeScale > 0 && myTransform.root.CompareTag(GameManager_References._playerTag))
            {
                ...
                {
                    if (bl_MobileInput.GetButton(attackButtonName))
                    {
                        Debug.Log("Full Auto");
                        AttemptAttack();
                    }
        ...
        {
            if (bl_MobileInput.GetButton(reloadButtonName) && Time.timeScale > 0 && myTransform.root.CompareTag(GameManager_References._playerTag))
            {
        ...
        {
            if (bl_MobileInput.GetButtonUp(burstFireButtonName) && Time.timeScale > 0 && myTransform.root.CompareTag(GameManager_References._playerTag))
            {
